...
public abstract class Math_model {
    ...
    public static void calculate() throws Exception {
        List<List<Double>> mat = Parse_json.input();
        List<Double> determinants = new ArrayList<>();
        for (int i = 1; i <= mat.size(); i++) {
            determinants.add(determinantOfMatrix(angleCofactor(mat, i), i));
        }
        boolean is_pos = true;
        boolean is_neg = determinants.get(0) <= 0;
        StringBuilder log = new StringBuilder("Determinants:");
        for (Double determinant : determinants) {
            log.append(" det").append(determinants.indexOf(determinant) + 1).append(" = ").append(determinant);
        }
        log.append("\n");
        for (Double determinant : determinants) {
            if (determinant < 0) {
                log.append(" det").append(determinants.indexOf(determinant) + 1).append(" is less than zero, so form is not positive.\n");
                is_pos = false;
                break;
            }
        }
        if (!is_pos && is_neg) {
            for (int i = 1; i < determinants.size(); i++) {
                if (i % 2 == 0 && determinants.get(i) < 0) {
                    log.append("det of odd index ").append(i).append(" is less than zero, so form is not negative.\n");
                    is_neg = false;
                    break;
                }
            }
        ...
        if (is_pos && !is_neg) {
            log.append(". None of the determinants is less than zero, so form is positive.\n");
            result = "Positive";
        }
        if (is_neg && !is_pos) {
            log.append("Odd index determinants are positive numbers, " +
                    "even index determinants are negative numbers, so form is negative.");
            result = "Negative";
        }
        if (!is_neg && !is_pos) {
            log.append("Form is neither positive or negative, so it is alternating.\n");
            result = "Altering";
        }
        Logs_writer.write_log(log.toString(), mat, UUID.randomUUID().toString(), result);
